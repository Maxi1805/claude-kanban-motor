...
REPOS=(
  ...
  "corpus-app/Ghost Ghost" "corpus/sqlalchemy sqlalchemy" "corpus-app/netbox netbox"
  "corpus-app/chatwoot chatwoot" "corpus/guava guava" "corpus-app/jenkins jenkins"
...
for entry in "${REPOS[@]}"; do
  ...
  echo "== $nombre" >&2
  CK_CUPOS="${CK_CUPOS:-3}" ./scripts/con-analisis.sh timeout 3000 npx tsx scripts/au4-dump2.mts "$dir" "$nombre" "$SALIDA/$nombre.json" 2>&1 | tail -3
done
echo "== FIN" >&2
