/**
 * OLA AM, FRENTE AM2 (copia de an1-embudo-b.mts, misma lógica, ÁRBOL CONGELADO src0) — EL EMBUDO Y LOS RASGOS DE FORMA DE LAS TRES ANCLAS DE
 * STRATEGY.
 *
 ...
 *
 * Uso: npx tsx scripts/an1-embudo-b.mts <dir-repo> <traza.json> <volcado.json>
 */
import { writeFileSync } from "node:fs";

import { analyzeRepo } from "../scratchpad-an1/srcB/server/services/code-analyzer.js";
import { stableFindingId } from "../scratchpad-an1/srcB/server/services/code-finding-ids.js";
import { direccionesDeFila } from "../scratchpad-an1/srcB/server/services/detect/precision/direccion-hipotesis.js";
import { startStrategyTrace, takeStrategyTrace } from "../scratchpad-an1/srcB/server/services/hypotheses/strategy.js";

const [, , dir, outTraza, outVolcado] = process.argv;
if (!dir || !outTraza || !outVolcado) {
  console.error("Uso: npx tsx scripts/an1-embudo-b.mts <dir-repo> <traza.json> <volcado.json>");
  process.exit(1);
}

const t0 = performance.now();
startStrategyTrace();
const a = await analyzeRepo({ dir, repoName: "an1", limits: { maxFindings: "unlimited" } });
...
const primera = new Map<string, (typeof traza)[number]>();
const pasadas = new Map<string, number>();
for (const e of traza) {
  ultima.set(e.findingId, e);
  if (!primera.has(e.findingId)) primera.set(e.findingId, e);
...
const filas = [...ultima.values()].map((e) => ({
  ...e,
  /* OLA AN · AN1 — la MISMA sonda de nivel 2 tal como la vio la PRIMERA pasada
   * (dentro de `analyzeFile`, `ctx.neighborhood === EMPTY_NEIGHBORHOOD`). Es la
   * mitad de la pregunta de MECANISMO: sin este contraste no se puede distinguir
   * "el canal está vacío" de "el canal no llega acá". */
  nivel2P1: primera.get(e.findingId)?.nivel2 ?? null,
  withGraphP1: primera.get(e.findingId)?.withGraph ?? null,
